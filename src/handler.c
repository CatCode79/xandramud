...
/*
 * Room record: For less than 5 people in room create a new record. Else use the oldest one.
 */
...
/*
 * Returns race number, delle razze mob
 */
int race_lookup( const char *name )
{
	...
	{
		/* Controlla sia il nome in inglese che quello in italiano */
		if ( !str_prefix(name, race_table[race].name)
		|| !str_prefix(name, race_table[race].translate)
		|| !str_prefix(name, race_table[race].ftranslate) )
			return( race );
	}
...
bool cabal_ok( CHAR_DATA *ch, sh_int sn )
{
	...
	for ( i = 1; i < MAX_CABAL; i++ )
	{
		...
		{
			send_to_char( "Non riesci piu' a percepire il potere della Cabala dentro di te.\n\r", ch );
			return( FALSE );
		}
	}
...
int item_lookup( const char *name )
{
	...
	{
		if ( !str_prefix(name, item_table[x].name) )   	/* nome inglese */
			return( item_table[x].type );
	}
...
{
	if ( is_name(obj->material, material) )
		return( TRUE );
	else
		return( FALSE );
}
...
/*
 * Ritorna il nome italiano di un materiale
 */
char *material_name( const char *material )
{
	int x;
	/*~~~~*/
	...
	{
		/* cerca la parola in inglese */
		if ( !str_prefix(material, material_table[x].name) )
			return material_table[x].translate;
	}
...
bool is_metal( OBJ_DATA *obj )
{
	...
	{
		/* cerca la parola in inglese */
		if ( is_name(obj->material, material_table[x].name) && material_table[x].flag & MATERIAL_METAL )
			return TRUE;
	}
...
bool is_melted( OBJ_DATA *obj )
{
	...
	{
		/* cerca la parola in inglese */
		if ( is_name(obj->material, material_table[x].name) && material_table[x].flag & MATERIAL_MELTED )
			return TRUE;
	}
...
bool cant_float( OBJ_DATA *obj )
{
	...
	{
		/* cerca la parola in inglese */
		if ( is_name(obj->material, material_table[x].name) && material_table[x].flag & MATERIAL_CANTFLOAT )
			return TRUE;
	}
...
bool may_float( OBJ_DATA *obj )
{
	...
	{
		/* cerca la parola in inglese */
		if ( is_name(obj->material, material_table[x].name) && material_table[x].flag & MATERIAL_FLOAT )
			return TRUE;
	}
...
int class_lookup( const char *name )
{
	...
	for ( class = 0; class < MAX_CLASS; class++ )
	{
		if ( !str_prefix(name, class_table[class].name)
		  || !str_prefix(name, class_table[class].fname) )
			return (class);
	}
...
/*
 * for immunity, vulnerabiltiy, and resistant the 'globals' (magic and weapons) may be overriden three other cases
 *    -- wood, silver, and iron -- are checked in fight.c
 */
int check_immune( CHAR_DATA *ch, int dam_type )
{
	...
	if ( !IS_MOB(ch) && get_curr_stat(ch, CHA) < 18 && dam_type == DAM_CHARM )
		immune = IS_VULNERABLE;
...
int get_skill( CHAR_DATA *ch, int sn )
{
	...
	else			/* mobiles */
	{
		if ( skill_table[sn].spell_fun != spell_null )
			...
			skill = 30 + ch->level;
		else if ( sn == gsn_third_attack && (IS_SET(ch->act, ACT_WARRIOR) || IS_SET(ch->act, ACT_THIEF)) )
			skill = 30 + ch->level / 2;
			...
			skill = 20 + 3 * ch->level;
		else if ( sn == gsn_grip && (IS_SET(ch->act, ACT_WARRIOR) || IS_SET(ch->act, ACT_THIEF)) )
			skill = ch->level;
	...
	if ( ch->point[LIFE] < (ch->max_point[LIFE] * 0.6) )
		skill = 9 * skill / 10;
...
void reset_char( CHAR_DATA *ch )
{
	...
	ch->max_point[LIFE] = ch->pcdata->perm_point[LIFE];
	ch->max_point[MANA] = ch->pcdata->perm_point[MANA];
	ch->max_point[MOVE] = ch->pcdata->perm_point[MOVE];

	/*
	 * ch->point[LIFE] = ch->max_point[LIFE]; ch->point[MANA] = ch->max_point[MANA]; ch->point[MOVE] =
	 * ch->max_point[MOVE];
	 */
	/* a little hack */
	ch->extracted = TRUE;
...
int get_max_train( CHAR_DATA *ch, int stat )
{
	...
	max = ( 20 + pc_race_table[ORG_RACE(ch)].stat[stat] + /* ORG_RACE && RACE serdar */ class_table[ch->class].stat[stat] );
...
/*
 * command for returning max training score for do_train and stat2train in comm.c
 */
int get_max_train2( CHAR_DATA *ch, int stat )
{
	...
	max = ( 20 + pc_race_table[ORG_RACE(ch)].stat[stat] + class_table[ch->class].stat[stat] );
...
int can_carry_n( CHAR_DATA *ch )
{
	...
	return( MAX_WEAR + get_curr_stat(ch, DEX) - 10 + ch->size );
}
...
int can_carry_w( CHAR_DATA *ch )
{
	...
	return( str_app[get_curr_stat(ch, STR)].carry * 10 + ch->level * 25 );
}
...
void affect_modify( CHAR_DATA *ch, AFFECT_DATA *paf, bool fAdd )
{
	...
	if ( fAdd )
	{
		...
		case TO_AFFECTS:
			...
			if ( IS_SET(paf->bitvector, AFF_FLYING) && !IS_MOB(ch) )
		...
		case TO_RACE:
			ch->race = paf->modifier < MAX_PC_RACE ? paf->modifier : 1;
			REMOVE_BIT( ch->affected_by, race_table[ORG_RACE(ch)].det );
			SET_BIT( ch->affected_by, race_table[ch->race].det );
			...
			SET_BIT( ch->imm_flags, race_table[ch->race].imm );
			REMOVE_BIT( ch->res_flags, race_table[ORG_RACE(ch)].res );
			SET_BIT( ch->res_flags, race_table[ch->race].res );
			REMOVE_BIT( ch->vuln_flags, race_table[ORG_RACE(ch)].vuln );
			SET_BIT( ch->vuln_flags, race_table[ch->race].vuln );
	...
	else
	{
		...
		case TO_RACE:
			REMOVE_BIT( ch->affected_by, race_table[ch->race].det );
			SET_BIT( ch->affected_by, race_table[ORG_RACE(ch)].det );
			REMOVE_BIT( ch->affected_by, race_table[ch->race].aff );
			SET_BIT( ch->affected_by, race_table[ORG_RACE(ch)].aff );
			REMOVE_BIT( ch->imm_flags, race_table[ch->race].imm );
			SET_BIT( ch->imm_flags, race_table[ORG_RACE(ch)].imm );
			REMOVE_BIT( ch->res_flags, race_table[ch->race].res );
			SET_BIT( ch->res_flags, race_table[ORG_RACE(ch)].res );
			REMOVE_BIT( ch->vuln_flags, race_table[ch->race].vuln );
	...
	switch ( paf->location )
	{
	default:					bug( "Affect_modify: locazione sconosciuta %d.", paf->location ); return;
	case APPLY_NONE:			break;
	case APPLY_STR:				ch->mod_stat[STR] += mod; break;
	case APPLY_DEX:				ch->mod_stat[DEX] += mod; break;
	case APPLY_INT:				ch->mod_stat[INT] += mod; break;
	case APPLY_WIS:				ch->mod_stat[WIS] += mod; break;
	case APPLY_CON:				ch->mod_stat[CON] += mod; break;
	case APPLY_CHA:				ch->mod_stat[CHA] += mod; break;
	case APPLY_CLASS:			break;
	case APPLY_LEVEL:			break;
	case APPLY_AGE:				ch->played += age_to_num( mod ); break;
	case APPLY_HEIGHT:			break;
	case APPLY_WEIGHT:			break;
	case APPLY_MANA:			ch->max_point[MANA] += mod; break;
	case APPLY_LIFE:			ch->max_point[LIFE] += mod; break;
	case APPLY_MOVE:			ch->max_point[MOVE] += mod; break;
	...
	if ( !IS_MOB(ch) && !ch->extracted )
	{
		static int	depth;
		/*~~~~~~~~~~~~~~*/

		if ( (hold = get_eq_char(ch, WEAR_BOTH)) != NULL && get_obj_weight(hold) > str_app[get_curr_stat(ch, STR)].carry )
		{
			if ( depth == 0 )
			{
				depth++;
				act( "Perdi la presa sulla tua arma.", ch, hold, NULL, TO_CHAR, POS_REST );
				act( "L'arma di $n finisce per terra.", ch, hold, NULL, TO_ROOM, POS_REST );
				obj_from_char( hold );
		...
		if ( (hold = get_eq_char(ch, WEAR_RIGHT)) != NULL && get_obj_weight(hold) > str_app[get_curr_stat(ch, STR)].carry )
		{
			if ( depth == 0 )
			{
				depth++;
		...
		if ( (hold = get_eq_char(ch, WEAR_LEFT)) != NULL && get_obj_weight(hold) > str_app[get_curr_stat(ch, STR)].carry )
		{
			if ( depth == 0 )
			{
				depth++;
...
void affect_to_obj( OBJ_DATA *obj, AFFECT_DATA *paf )
{
	...
	*paf_new = *paf;
	...
	if ( paf->bitvector )
	{
		...
		case TO_WEAPON: if ( obj->item_type == ITEM_WEAPON ) SET_BIT( obj->value[4], paf->bitvector ); break;
		}
	}
}
...
void affect_remove( CHAR_DATA *ch, AFFECT_DATA *paf )
{
	...
	if ( ch->affected == NULL )
	{
		bug( "Affect_remove: nessun affect.", 0 );
		return;
	}
	...
	else
	{
		...
		if ( prev == NULL )
		{
			bug( "Affect_remove: impossibile trovare paf.", 0 );
			return;
		}
	}
...
void affect_remove_obj( OBJ_DATA *obj, AFFECT_DATA *paf )
{
	...
	{
		bug( "Affect_remove_object: nessun affect.", 0 );
		return;
	}
	...
	if ( paf->bitvector )
	{
		...
		{
		case TO_OBJECT:
			REMOVE_BIT( obj->extra_flags, paf->bitvector ); break;
		case TO_WEAPON:
			if ( obj->item_type == ITEM_WEAPON )
				REMOVE_BIT( obj->value[4], paf->bitvector ); break;
		}
	}
	...
	else
	{
		...
		if ( prev == NULL )
		{
			bug( "Affect_remove_object: impossibile trovare paf.", 0 );
			return;
		}
	}
...
void char_from_room( CHAR_DATA *ch )
{
	...
	if ( ch->in_room == NULL )
	{
		bug( "Char_from_room: ch->in_room e' NULL.", 0 );
		return;
	}
...
/*	&& obj->item_type == ITEM_LIGHT && obj->value[2] != 0 */
	...
	else
	{
		...
		if ( prev == NULL )
			bug( "Char_from_room: ch non trovato.", 0 );
	}
	...
	while ( IS_AFFECTED(ch, AFF_PLAGUE) )
	{
		...
		for ( vch = ch->in_room->people; vch != NULL; vch = vch->next_in_room )
		{
			...
			&&	number_bits(6) == 0 )
			{
				...
				act( "$n rabbrividisce e sembra molto ammalato.", vch, NULL, NULL, TO_ROOM, POS_REST );
...
void obj_from_char( OBJ_DATA *obj )
{
	...
	else
	{
		...
		if ( prev == NULL )
			bug( "Obj_from_char: obj non nella lista.", 0 );
	}
...
void equip_char( CHAR_DATA *ch, OBJ_DATA *obj, int iWear )
{
	...
	{
		bug( "Equip_char: gia' equipaggiato nella locazione wear (%d).", iWear );
		return;
	}
	...
	||	(IS_OBJ_STAT(obj, ITEM_ANTI_NEUTRAL) && IS_NEUTRAL(ch)) )
	{
		...
		act( "Vieni fulminat$x da $p e l'abbandoni.", ch, obj, NULL, TO_CHAR, POS_REST );
		act( "$n viene fulminat$x da $p e l'abbandona.", ch, obj, NULL, TO_ROOM, POS_REST );
		obj_from_char( obj );
	...
	&&	ch->in_room != NULL
	&&	((obj->item_type == ITEM_LIGHT && obj->value[2] != 0) || (iWear == WEAR_HEAD && IS_OBJ_STAT(obj, ITEM_GLOW))) )
		++ch->in_room->light;
...
void unequip_char( CHAR_DATA *ch, OBJ_DATA *obj )
{
	...
	if ( obj->wear_loc == WEAR_NONE )
	{
		bug( "Unequip_char: gia' equipaggiato.", 0 );
		return;
	}
	...
	if (get_light_char(ch) == NULL
	&&	ch->in_room != NULL
	&&	((obj->item_type == ITEM_LIGHT && obj->value[2] != 0) || (old_wear == WEAR_HEAD && IS_OBJ_STAT(obj, ITEM_GLOW)))
	&&	ch->in_room->light > 0 )
	{
		--ch->in_room->light;
	}
...
void obj_from_room( OBJ_DATA *obj )
{
	...
	else
	{
		...
		if ( prev == NULL )
		{
			bug( "Obj_from_room: obj non trovato.", 0 );
			return;
		}
	}
	...
	if ( obj->pIndexData->vnum < 600 )
	{
		...
		if ( i < MAX_CABAL )
		{
			...
			if ( i < MAX_CABAL )
			{
				...
				{
					act( "$p perdi la tua trasparenza e ritorni solid$x.", pRoomIndex->people, obj, NULL, TO_CHAR, POS_REST );
					act( "$p perde la sua trasparenza e ritorna solid$x.", pRoomIndex->people, obj, NULL, TO_ROOM, POS_REST );
				}
			}
		}
	}
...
void obj_from_obj( OBJ_DATA *obj )
{
	...
	{
		bug( "Obj_from_obj: obj_from e' NULL.", 0 );
		return;
	}
	...
	else
	{
		...
		if ( prev == NULL )
		{
			bug( "Obj_from_obj: obj non trovato.", 0 );
			return;
		}
	}
...
void extract_obj_1( OBJ_DATA *obj, bool count )
{
	...
	{
		sprintf( buf, "Attenzione! Estrazione di %s, vnum %d.", obj->name, obj->pIndexData->vnum );
		bug( buf, 0 );
		return;					/* if it's already been extracted, something bad is going on */
	}
	else
		obj->extracted = TRUE;	/* if it hasn't been extracted yet, now it's being extracted. */
	...
	if ( obj->pIndexData->vnum == OBJ_VNUM_MAGIC_JAR )
	{
		...
		for ( wch = char_list; wch != NULL; wch = wch->next )
		{
			...
			if ( is_name(obj->name, wch->name) )
			{
				...
				send_to_char( "Hai ripreso il tuo spirito!\n\r", wch );
				break;
			}
		}
	}
	...
	else
	{
		...
		if ( prev == NULL )
		{
			bug( "Extract_obj: obj %d non trovato.", obj->pIndexData->vnum );
			return;
		}
	}
...
void extract_char_org( CHAR_DATA *ch, bool fPull, bool Count )
{
	...
	if ( fPull )					/* only for total extractions should it check */
	{
		...
		{
			sprintf( buf, "Extract_char_org: Attenzione! Estrazione di %s.", ch->name );
			bug( buf, 0 );
			return;					/* if it's already been extracted, something bad is going on */
		}
		else
			ch->extracted = TRUE;	/* if it hasn't been extracted yet, now it's being extracted. */
	}

	if ( ch->in_room == NULL )
	{
		bug( "Extract_char: ch->in_room e' NULL.", 0 );
		return;
	}
	...
	else
	{
		...
		if ( prev == NULL )
		{
			bug( "Extract_char_org: char non trovato.", 0 );
			return;
		}
	}
...
CHAR_DATA *get_char_room( CHAR_DATA *ch, char *argument )
{
	...
	if ( !str_cmp(arg, "self") || !str_cmp(arg, "se stesso") )
		return( ch );
	if ( !str_cmp(arg, "ugly") || !str_cmp(arg, "brutto") )
		ugly = 1;
...
/*
 * Find a char in the room. Chronos uses in act_move.c
 */
CHAR_DATA *get_char_room2( CHAR_DATA *ch, ROOM_INDEX_DATA *room, char *argument, int *number )
{
	...
	if ( !str_cmp(argument, "ugly") || !str_cmp(argument, "brutto") )
		ugly = 1;
...
OBJ_DATA *create_money( int gold, int silver )
{
	...
	{
		bug( "Create_money: moneta in quantita' zero o negativa.", UMIN(gold, silver) );
		gold = UMAX( 1, gold );
...
/*
 * Return # of objects which an object counts as. Thanks to Tony Chamberlain for the correct recursive code here.
 */
...
/*
 * Vera se la stanza non ha uscite.
 */
bool room_is_dt( CHAR_DATA *ch )
{
	sh_int	i;
	/*~~~~~~*/
...
bool can_see_room( CHAR_DATA *ch, ROOM_INDEX_DATA *pRoomIndex )
{
	...
	if ( IS_SET(pRoomIndex->room_flags, ROOM_GODS_ONLY) && !IS_ADMIN(ch) )
		return( FALSE );

	if ( IS_SET(pRoomIndex->room_flags, ROOM_HEROES_ONLY) && !IS_ADMIN(ch) )
		return( FALSE );

	if ( IS_SET(pRoomIndex->room_flags, ROOM_NEWBIES_ONLY) && ch->level > LEVEL_NEWBIE && !IS_ADMIN(ch) )
		return( FALSE );
...
bool can_see( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	if ( (!IS_MOB(ch) && IS_SET(ch->act, PLR_HOLYLIGHT)) || (IS_MOB(ch) && IS_ADMIN(ch)) )
		return( TRUE );
	...
	if ( IS_AFFECTED(victim, AFF_INVISIBLE) && !CAN_DETECT(ch, DETECT_INVIS) )
		return( FALSE );

	if ( IS_AFFECTED(victim, AFF_IMP_INVIS) && !CAN_DETECT(ch, DETECT_IMP_INVIS) )
		return( FALSE );

	/* sneaking */
/*	if ( IS_AFFECTED(victim, AFF_SNEAK) && !CAN_DETECT(ch, DETECT_HIDDEN) && victim->fighting == NULL )
	{
		int chance;
...
*/
	...
	if ( IS_AFFECTED(victim, AFF_CAMOUFLAGE) && !CAN_DETECT(ch, ACUTE_VISION) )
		return( FALSE );

	if ( IS_AFFECTED(victim, AFF_HIDE) && !CAN_DETECT(ch, DETECT_HIDDEN) && victim->fighting == NULL )
		return( FALSE );

	if ( IS_AFFECTED(victim, AFF_FADE) && !CAN_DETECT(ch, DETECT_FADE) && victim->fighting == NULL )
		return( FALSE );
...
bool can_see_obj( CHAR_DATA *ch, OBJ_DATA *obj )
{
	...
	if ( IS_SET(obj->extra_flags, ITEM_INVIS) && !CAN_DETECT(ch, DETECT_INVIS) )
		return( FALSE );

	if ( IS_SET(obj->extra_flags, ITEM_BURIED) && !IS_ADMIN(ch) )
		return( FALSE );
...
char *item_type_name( OBJ_DATA * obj )
{
	...
	for ( x = 0; item_table[x].name != NULL; x++ )
	{
		if ( obj->item_type == item_table[x].type )
			return( item_table[x].translate );
	}
...
char *affect_loc_name( int location )
{
	switch ( location )
	{
	case APPLY_NONE:			return( "nessuno" );
	case APPLY_STR:				return( "forza" );
	case APPLY_DEX:				return( "destrezza" );
	case APPLY_INT:				return( "intelligenza" );
	case APPLY_WIS:				return( "saggezza" );
	case APPLY_CON:				return( "costituzione" );
	case APPLY_CHA:				return( "carisma" );
	case APPLY_CLASS:			return( "classe" );
	case APPLY_LEVEL:			return( "livello" );
	case APPLY_AGE:				return( "eta'" );
	case APPLY_MANA:			return( "mana" );
	case APPLY_LIFE:			return( "vita" );
	case APPLY_MOVE:			return( "movimento" );
	case APPLY_GOLD:			return( "oro" );
	case APPLY_EXP:				return( "esperienza" );
	case APPLY_AC:				return( "class armatura" );
	case APPLY_HITROLL:			return( "hit" );
	case APPLY_DAMROLL:			return( "dam" );
	case APPLY_SIZE:			return( "taglia" );
	case APPLY_SAVES:			return( "protezione" );
	case APPLY_SAVING_ROD:		return( "protezione vs bastoni" );
	case APPLY_SAVING_PETRI:	return( "protezione vs pietrificazione" );
	case APPLY_SAVING_BREATH:	return( "protezione vs soffio" );
	case APPLY_SAVING_SPELL:	return( "protezione vs incantesimo" );
	case APPLY_SPELL_AFFECT:	return( "nessuno" );
	}

	bug( "Affect_location_name: locazione sconosciuta %d.", location );
	return( "(sconosciuta)" );
}
...
char *extra_bit_name( int extra_flags )
{
	...
	if ( extra_flags & ITEM_SLAYGOOD )		strcat( buf, " slaygood" );
	if ( extra_flags & ITEM_SLAYEVIL )		strcat( buf, " slayevil" );
	if ( extra_flags & ITEM_SLAYNEUTRAL )	strcat( buf, " slayneutral" );
...
char *act_bit_name( int act_flags )
{
	...
	if ( IS_SET(act_flags, ACT_IS_MOB) )
	{
		strcat( buf, " npc" );
	...
	else
	{
		...
		if ( act_flags & PLR_ENGLISH )			strcat( buf, " english" );	/* Xandra */
...
char *comm_bit_name( int comm_flags )
{
	...
	if ( comm_flags & COMM_NOYELL )			strcat( buf, " no_yell" );
...
int cabal_lookup( const char *argument )
{
	...
	{
		if ( LOWER(argument[0]) == LOWER(cabal_table[cabal].short_name[0]) && !str_prefix(argument, cabal_table[cabal].short_name) )
			return( cabal );
	}
...
bool isn_dark_safe( CHAR_DATA *ch )
{
	...
	for ( rch = ch->in_room->people; rch != NULL; rch = rch->next_in_room )
	{
		if ( (light = get_light_char(rch)) != NULL && light->item_type == ITEM_LIGHT && IS_OBJ_STAT(light, ITEM_MAGIC) )
		{
			light_exist = 1;
			break;
		}
...
void affect_modify_room( ROOM_INDEX_DATA *room, AFFECT_DATA *paf, bool fAdd )
{
	...
	switch ( paf->location )
	{
	default:				bug( "Affect_modify_room: locazione sconosciuta %d.", paf->location ); return;
...
void affect_remove_room( ROOM_INDEX_DATA *room, AFFECT_DATA *paf )
{
	...
	{
		bug( "Affect_remove_room: nessun affect.", 0 );
		return;
	}
	...
	else
	{
		...
		if ( prev == NULL )
		{
			bug( "Affect_remove_room: impossibile trovare paf.", 0 );
			return;
		}
	}
	...
	if ( !room->affected )
	{
		...
		else
		{
			...
			if ( prev == NULL )
			{
				bug( "Affect_remove_room: impossibile trovare room.", 0 );
				return;
			}
		}
...
bool is_safe_rspell_nom( int level, CHAR_DATA *victim )
{
	...
	if ( victim->level < LEVEL_NEWBIE && !IS_MOB(victim) )
		return( TRUE );

	if ( !IS_MOB(victim) && (victim->last_death_time != -1 && current_time - victim->last_death_time < 600) )
		return( TRUE );

	if ( !IS_MOB(victim) && ((level >= victim->level + 5) || (victim->level >= level + 5)) )
		return( TRUE );
...
bool is_safe_rspell( int level, CHAR_DATA *victim )
{
	...
	{
		act( "Gli Dei ti proteggono.", victim, NULL, NULL, TO_CHAR, POS_REST );
		act( "Gli Dei proteggono $n dagli incantesimi del luogo.", victim, NULL, NULL, TO_ROOM, POS_REST );
		return( TRUE );
	}
	else
		return( FALSE );
...
void raffect_to_char( ROOM_INDEX_DATA *room, CHAR_DATA *ch )
{
	...
	if ( IS_ROOM_AFFECTED(room, AFF_ROOM_L_SHIELD) )
	{
		...
		{
			send_to_char( "Lo scudo protettivo attorno al luogo ti impedisce di entrarvi.\n\r", ch );
			act( "$N e' entrato nel luogo.", vch, NULL, ch, TO_CHAR, POS_REST );
			do_wake( vch, "" );
			...
			if ( !is_safe_rspell(paf->level, ch) )
			{
				if ( CAN_DETECT(ch, ADET_GROUNDING) )
				{
					...
					act( "A lightning bolt fizzles at $N's foes.", ch, NULL, ch, TO_ROOM, POS_REST );
				}
				else
				{
	...
	if ( IS_ROOM_AFFECTED(room, AFF_ROOM_SHOCKING) )
	{
		...
		send_to_char( "Le onde elettriche nella stanza ti fulminano.\n\r", ch );
	...
	{
		send_to_char( "Una trappola, piazzata da qualcuno, ti impedisce di andare oltre.\n\r", ch );
	...
	if ( IS_ROOM_AFFECTED(room, AFF_ROOM_SLOW) || IS_ROOM_AFFECTED(room, AFF_ROOM_SLEEP) )
		act( "$CC'e' una strana polvere che riempie l'aria.$c", ch, NULL, NULL, TO_CHAR, POS_SLEEP, YELLOW );
}
...
void raffect_back_char( ROOM_INDEX_DATA *room, CHAR_DATA *ch )
{
	if ( IS_ROOM_AFFECTED(room, AFF_ROOM_L_SHIELD) )
	{
		...
		if ( (sn = skill_lookup("lightning shield")) == -1 )
		{
			bug( "Raffect_back_char: sn errato per il lightning shield", 0 );
			return;
		}
...
int count_charmed( CHAR_DATA *ch )
{
	...
	{
		if ( IS_AFFECTED(gch, AFF_CHARM) && gch->master == ch && ch->pet != gch )
			count++;
	}

	if ( count >= (get_curr_stat(ch, INT) / 6) + (ch->level / 45) )	/* ex-macro MAX_CHARM */
	{
		send_to_char( "Controlli gia' il massimo di creature che puoi!\n\r", ch );
		return( count );
	}
...
void remove_mind( CHAR_DATA *ch, char *str )
{
	...
	if ( !IS_MOB(ch) || ch->in_room == NULL || mind == NULL || !is_name(str, mind) )
		return;
	...
	do_say( ch, "Finalmente, ho ottenuto vendetta!" );
	free_string( ch->in_mind );
	ch->in_mind = str_dup( buf );
	if ( is_number(buf) )
		back_home( ch );
...
void back_home( CHAR_DATA *ch )
{
	...
	if ( ch->fighting == NULL && location != ch->in_room )
	{
		...
		act( "$n si materializza nella stanza.", ch, NULL, NULL, TO_ROOM, POS_REST );
...
CHAR_DATA *find_char( CHAR_DATA *ch, char *argument, int door, int range )
{
	...
	{
		bug( "Find_char: door errata: %d", door );
		send_to_char( "Non vedi nulla laggiu'.\n\r", ch );
		return( NULL );
	}
	...
	while ( range > 0 )
	{
		...
		{
			send_to_char( "La strada che hai scelto ti impedisce di passare.\n\r", ch );
			return( NULL );
		}
...
CHAR_DATA *get_char_spell( CHAR_DATA *ch, char *argument, int *door, int range )
{
	...
	if ( i == 0 || (*door = find_exit(ch, buf, FALSE)) == -1 )
	...
	if ( IS_MOB(victim) && victim->position != POS_DEAD )
	{
		...
		for ( i = 0; i < 1000; i++ )
		{
			...
			if ( victim->in_room == temp )
				break;
			if ( (pExit = temp->exit[door]) == NULL || (temp = pExit->to_room) == NULL )
			{
		...
		while ( --range > 0 )
		{
			...
			if ( (pExit = temp->exit[opdoor]) == NULL || (temp = pExit->to_room) == NULL )
			{
				sprintf( log_buf, "Path to track: Range: %d Room: %d opdoor:%d", range, temp->vnum, opdoor );
				bug( log_buf, 0 );
				return;
			}
		}
...
OBJ_DATA *get_wield_char( CHAR_DATA *ch, bool second )
{
	...
	for ( obj = ch->carrying; obj != NULL; obj = obj->next_content )
	{
		if ( obj->item_type == ITEM_WEAPON )
		{
			if ( second )
			{
				if ( (obj->wear_loc == WEAR_RIGHT && LEFT_HANDER(ch)) || (obj->wear_loc == WEAR_LEFT && RIGHT_HANDER(ch)) )
					return( obj );
			}
			else
			{
...
OBJ_DATA *get_shield_char( CHAR_DATA *ch )
{
	...
	for ( obj = ch->carrying; obj != NULL; obj = obj->next_content )
	{
		if ( (obj->wear_loc == WEAR_LEFT || obj->wear_loc == WEAR_RIGHT || obj->wear_loc == WEAR_BOTH) && CAN_WEAR(obj, ITEM_WEAR_SHIELD) )
			return( obj );
	}
...
OBJ_DATA *get_hold_char( CHAR_DATA *ch )
{
	...
	for ( obj = ch->carrying; obj != NULL; obj = obj->next_content )
	{
		if ( (obj->wear_loc == WEAR_LEFT || obj->wear_loc == WEAR_RIGHT || obj->wear_loc == WEAR_BOTH) && CAN_WEAR(obj, ITEM_HOLD) )
			return( obj );
	}
...
OBJ_DATA *get_light_char( CHAR_DATA *ch )
{
	...
	for ( obj = ch->carrying; obj != NULL; obj = obj->next_content )
	{
		if ( (obj->item_type == ITEM_LIGHT
			&&	obj->value[2] != 0
			&&	(obj->wear_loc == WEAR_LEFT || obj->wear_loc == WEAR_RIGHT || obj->wear_loc == WEAR_BOTH))
		|| (obj->wear_loc == WEAR_HEAD && IS_OBJ_STAT(obj, ITEM_GLOW)) )
			return( obj );
	}
...
bool is_wielded_char( CHAR_DATA *ch, OBJ_DATA *obj )
{
	...
	{
		if ( (w->wear_loc == WEAR_LEFT || w->wear_loc == WEAR_RIGHT || w->wear_loc == WEAR_BOTH) && CAN_WEAR(w, ITEM_WIELD) && w == obj )
			return( TRUE );
	}
