...
void ban_site( CHAR_DATA *ch, char *argument, bool fPerm )
{
	...
	if ( arg1[0] == '\0' )
	{
		if ( ban_list == NULL )
		{
			send_to_char( "Nessun indirizzo bannato per ora.\n\r", ch );
			return;
		}

		buffer = new_buf();

		add_buf( buffer, "Indirizzo     Livello  Tipo     Stato\n\r" );
		for ( pban = ban_list; pban != NULL; pban = pban->next )
		{
			sprintf( buf2, "%s%s%s",
			...
			sprintf( buf, "%-12s    %-3d  %-7s  %s\n\r",
				buf2,
				pban->level,
				IS_SET(pban->ban_flags, BAN_NEWBIES) ? "newbie" : IS_SET(pban->ban_flags, BAN_PLAYER) ?
					"giocatore" : IS_SET(pban->ban_flags, BAN_PERMIT) ?
					"permit" : IS_SET(pban->ban_flags, BAN_ALL) ? "tutti" : "",
	...
	if		( arg2[0] == '\0' || !str_prefix(arg2, "all") )		type = BAN_ALL;
	else if ( !str_prefix(arg2, "newbies") )					type = BAN_NEWBIES;
	else if ( !str_prefix(arg2, "player") )						type = BAN_PLAYER;
	else if ( !str_prefix(arg2, "permit") )						type = BAN_PERMIT;
	else
	{
		send_to_char( "I tipi di ban disponibili sono: tutti, newbie, giocatore, e permit.\n\r", ch );
		return;
	}
	...
	if ( strlen(name) == 0 )
	{
		send_to_char( "Devi bannare un indirizzo.\n\r", ch );
		return;
	}
	...
	for ( pban = ban_list; pban != NULL; prev = pban, pban = pban->next )
	{
		if ( !str_cmp(name, pban->name) )
		{
			...
			{
				send_to_char( "Questo ban e' stato imposto da chi ha piu' poteri dei tuoi.\n\r", ch );
				return;
			}
			else
			{
...
void do_allow( CHAR_DATA *ch, char *argument )
{
	...
	if ( arg[0] == '\0' )
	{
		send_to_char( "Quale indirizzo vuoi rimuovere dalla lista dei ban?\n\r", ch );
		return;
	}
	...
	for ( curr = ban_list; curr != NULL; prev = curr, curr = curr->next )
	{
		if ( !str_cmp(arg, curr->name) )
		{
			...
			{
				send_to_char( "Non hai poteri a sufficienza per farlo.\n\r", ch );
				return;
			}
			...
			free_ban( curr );
			sprintf( buf, "Il ban su %s e' stato tolto.\n\r", arg );
			send_to_char( buf, ch );
			save_bans();
			return;
		}
	...
	send_to_char( "L'indirizzo non e' bannato.\n\r", ch );
}
