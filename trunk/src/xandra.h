...
#define XANDRA_PORT 5001

#define XANDRA_SINGLE_USER	FALSE
#define XANDRA_DNS_RESOLVE	FALSE
...
#define MSL				4608	/* MAX_STRING_LENGTH */
#define MIL				 256	/* MAX_INPUT_LENGTH	 */
#define PAGELEN			  22


/* ANSI Colors */
#define CLR(ch, color)	( IS_SET((ch)->act, PLR_COLOR) ) ? color : ""
...
#define WHITE_BOLD		"\033[37;0m"

#define BLINK			"\033[5m"
#define BOLD			"\033[1m"
#define NORMAL			"\033[0;37;40m"
 ...
 * Game parameters.
 */
#define XANDRA_REBOOT	  0
#define XANDRA_SHUTDOWN	  1
...
#define LEVEL_CREATOR		( MAX_LEVEL - 1 )
#define LEVEL_MASTER		( MAX_LEVEL - 2 )
#define LEVEL_ADMIN			( MAX_LEVEL - 3 )
#define LEVEL_AVATAR    	( MAX_LEVEL - 4 )
#define LEVEL_HERO			( MAX_LEVEL - 15 )
#define LEVEL_NEWBIE		( 5 )


#ifdef XANDRA_MACHINE
...
#define PULSE_WATER_FLOAT	( 6 * PULSE_PER_SECOND )
#define PULSE_TRACK			( 8 * PULSE_PER_SECOND )
#define PULSE_TICK		 	(75 * PULSE_PER_SECOND ) /* 54 secondi */
/* room_affect_update (not room_update) */
#define PULSE_RAFFECT		( 3 * PULSE_MOBILE )
#define PULSE_AREA			(330 * PULSE_PER_SECOND ) /* 296 secondi */
#define PULSE_DREAM			(90 * PULSE_PER_SECOND )
#define FIGHT_DELAY_TIME 	(30 * PULSE_PER_SECOND )
#define PULSE_AUCTION		(15 * PULSE_PER_SECOND ) /* 10 secondi */
 ...
 */
#define ART_FEMMINILE		( A )
#define ART_PLURALE			( B )
#define ART_INDETERMINATO	( C )
 ...
 * MAX_RELIGION in xandra.h  and make oprog for the tattoo.
 */
...
#define CON_GET_ENGLISH				20
#define CON_CREATE_DONE				21
#define CON_READ_NEWBIE				22
...
struct class_type
{
	...
	char	*fname;					/* nome femminile della classe */
	...
	sh_int	stat[MAX_STATS];		/* Stat modifiers */
...
struct attack_type
{
	char	*name;						/* name */
	char	*noun;						/* message */
	char	article;					/* tipo di articolo. Xandra */
...
struct race_type
{
	...
	char	*translate;					/* nome razza tradotto */
	char	*ftranslate;				/* nome razza femminile tradotto */
...
struct pc_race_type
{
	...
	char	*fname;						/* Nome femminile */
	int		points;						/* cost in exp of the race */
	sh_int	class_mult[MAX_CLASS];		/* exp multiplier for class, * 100 */
	char	*skills[5];					/* bonus skills for the race */
	sh_int	stat[MAX_STATS];			/* starting stats */
	sh_int	max_stat[MAX_STATS];		/* maximum stats */
	int		point[MAX_POINTS];			/* Massimo di bonus vita, mana, mov */
	int		max_point[MAX_POINTS];		/* Punti massimi di vita, mana, mov acquisibili da quella razza */
...
#define MOB_VNUM_ZOMBIE				21
#define MOB_VNUM_SKELETON			22
#define MOB_VNUM_SPECTRE			23
#define MOB_VNUM_BANSHEE			24
...
#define RACE_GIANT			( K )
#define RACE_FELAR			( L )
#define RACE_GITHYANKI		( M )
#define RACE_SATYR			( N )
#define RACE_TROLL			( O )	/* 15 */
#define RACE_TOP			( P )
 ...
 * Used in #MOBILES.
 */
#define ACT_IS_MOB			( A )	/* Auto set for mobs */
...
/* marry */
#define OBJ_VNUM_DIAMOND_RING		8
#define OBJ_VNUM_WEDDING_BAND		9
...
#define OBJ_VNUM_STONE				18	/* Stone vnum for sharpen */
...
#define OBJ_VNUM_ARMY_PONCHO		26

#define OBJ_VNUM_ARMY_THRONE		542
#define OBJ_VNUM_YVALADRIN_ALTAR		530
...
#define ITEM_PILL		26
#define ITEM_PROTECT	27	/* inutilizzato */
#define ITEM_MAP		28
...
#define ITEM_SLAYNEUTRAL    ( X )		/* slay */
#define ITEM_BURN_PROOF		( Y )
#define ITEM_NOUNCURSE		( Z )
#define ITEM_NOSELL			( aa )
#define ITEM_BURIED			( bb )
#define ITEM_SHARP			( cc )
#define ITEM_SLAYGOOD       ( dd )		/* slay */
#define ITEM_SLAYEVIL       ( ee )		/* slay */
...
#define APPLY_LEVEL			8	/* potenzialmente pericolosa */
#define APPLY_AGE			9
#define APPLY_HEIGHT		10
#define APPLY_WEIGHT		11
#define APPLY_MANA			12
#define APPLY_LIFE			13
#define APPLY_MOVE			14
#define APPLY_GOLD			15	/* potenzialmente pericolosa */
#define APPLY_EXP			16	/* potenzialmente pericolosa */
#define APPLY_AC			17
...
#define PLR_GHOST		( T )
#define PLR_ENGLISH		( U )
#define PLR_AUTOWEATHER ( V )
...
#define PLR_DENY				( X )
#define PLR_FREEZE				( Y )
#define PLR_LEFTHAND			( Z )	/* (GG) per la scelta della propria mano */
...
#define WIZ_DEATHS		( E )
#define WIZ_RESETS		( F )
#define WIZ_MOBDEATHS	( G )
#define WIZ_PENALTIES	( H )
#define WIZ_SACCING		( I )
#define WIZ_LEVELS		( J )
#define WIZ_SECURE		( K )
#define WIZ_SWITCHES	( L )
#define WIZ_SNOOPS		( M )
#define WIZ_RESTORE		( N )
#define WIZ_LOAD		( O )
#define WIZ_SPAM		( P )
#define WIZ_LIMITED		( Q )
...
struct char_data
{
	...
	long			id;
	sh_int			version;
	int				temp_num;		/* utilizzato per varie cose */
	char			*short_descr;
	char			*long_descr;
	char			*description;
	char			*prompt;
	...
	sh_int			wait;
	sh_int			daze;
	int				point[MAX_POINTS];
	int				max_point[MAX_POINTS];
	long			gold;
	long			silver;
	int				exp;
	long			act;
...
struct pc_data
{
	...
	time_t	last_news;
	time_t	last_changes;
	int		perm_point[MAX_POINTS];
	sh_int	true_sex;
	int		last_level;
	sh_int	condition[MAX_COND];
	sh_int	learned[MAX_SKILL];
	sh_int	points;					/* Xandra, trovare altro nome per non confenderli con i punteggi */
 ...
 * Materiali
 */
#define MATERIAL_METAL		( A )
#define MATERIAL_FLOAT		( B )
#define MATERIAL_CANTFLOAT	( C )
#define MATERIAL_MELTED		( D )
...
struct skill_type
{
	...
	char		article_damage;			/* genere grammaticale del messaggio di danno */
...
#define IS_GOOD( ch )				( (ch)->alignment >= 360 )
#define IS_EVIL( ch )				( (ch)->alignment <= -360 )
...
/*
 * OS-dependent declarations for Linux.
 */
char *crypt ( const char *key, const char *salt );


/*
 * The crypt(3) function is not available on some operating systems. In particular, the U.S. Government prohibits its
 * export from the United States to foreign countries. Turn on NOCRYPT to keep passwords in plain text.
 */
#if defined( NOCRYPT )
...
bool	controlla_vocale		( const char letter );			/* snippets di Onirik */
char	*scegli_articolo		( const char *parola, int tipo );	/* snippets di Onirik */
...
CD		*check_guard			( CHAR_DATA *ch, CHAR_DATA *mob );
...
CD		*get_char_room2			( CHAR_DATA *ch, ROOM_INDEX_DATA *room, char *argument, int *number );
...
CD		*find_char				( CHAR_DATA *ch, char *argument, int door, int range );
CD		*get_char_spell			( CHAR_DATA *ch, char *argument, int *door, int range );
void	path_to_track			( CHAR_DATA *ch, CHAR_DATA *victim, int door );
OD		*get_wield_char			( CHAR_DATA *ch, bool second );
OD		*get_hold_char			( CHAR_DATA *ch );
OD		*get_shield_char		( CHAR_DATA *ch );
OD		*get_light_char			( CHAR_DATA *ch );
OD		*get_weapon_char		( CHAR_DATA *ch, int wType );
...
CD		*MOUNTED				( CHAR_DATA *ch );
CD		*RIDDEN					( CHAR_DATA *ch );
